...
def _make_test_image(name='test.jpg', size=(900, 600), color='red'):
    """Build an in-memory JPEG SimpleUploadedFile for image-field tests."""
...
@override_settings(MEDIA_ROOT=tempfile.mkdtemp(prefix='dinify_test_media_'))
class MenuItemThumbnailTests(TestCase):
    """Verify image_thumbnail auto-generation, serializer exposure and backfill."""
    ...
    def _make_item(self, name='Thumb Test Item', image=None):
        return MenuItem.objects.create(
            name=name,
            section=self.section,
            primary_price=1000.0,
    ...
    def test_save_with_image_populates_thumbnail(self):
        item = self._make_item(image=_make_test_image())
        self.assertTrue(item.image_thumbnail)
        self.assertTrue(item.image_thumbnail.name.startswith('menu_items/thumbnails/'))
        self.assertTrue(item.image_thumbnail.name.endswith('_thumb.jpg'))

    def test_thumbnail_is_400x400_jpeg(self):
        from PIL import Image
        item = self._make_item(image=_make_test_image())
        item.image_thumbnail.open()
        try:
            img = Image.open(item.image_thumbnail)
            self.assertEqual(img.size, (400, 400))
            self.assertEqual(img.format, 'JPEG')
        finally:
            item.image_thumbnail.close()

    def test_changing_image_regenerates_thumbnail(self):
        item = self._make_item(image=_make_test_image(name='first.jpg', color='red'))
        first_name = item.image_thumbnail.name

        item.image = _make_test_image(name='second.jpg', color='blue')
        item.save()
        item.refresh_from_db()

        self.assertTrue(item.image_thumbnail)
        self.assertNotEqual(item.image_thumbnail.name, first_name)

    def test_save_succeeds_when_thumbnail_generation_fails(self):
        from unittest.mock import patch
        with patch(
            'restaurants_app.utils.image_optimizer.generate_thumbnail',
            side_effect=Exception('boom'),
        ):
            item = self._make_item(
                name='Crash Safe Item',
                image=_make_test_image(),
            )
        self.assertIsNotNone(item.pk)
        # Thumbnail never persisted because helper raised.
        self.assertFalse(item.image_thumbnail)

    def test_serializer_public_get_menu_item_includes_thumbnail(self):
        from restaurants_app.serializers import SerializerPublicGetMenuItem
        item = self._make_item(image=_make_test_image())
        data = SerializerPublicGetMenuItem(item).data
        self.assertIn('image_thumbnail', data)
        self.assertTrue(data['image_thumbnail'])
        self.assertIn('image', data)

    def test_serializer_handles_missing_thumbnail(self):
        from restaurants_app.serializers import SerializerPublicGetMenuItem
        item = MenuItem.objects.create(
            name='No Image Item',
            section=self.section,
        ...
        data = SerializerPublicGetMenuItem(item).data
        self.assertIn('image_thumbnail', data)
        self.assertFalse(data['image_thumbnail'])

    def test_upsell_item_serializer_includes_item_image_thumbnail(self):
        from restaurants_app.models import UpsellConfig, UpsellItem
        from restaurants_app.serializers import UpsellItemSerializer
        restaurant = Restaurant.objects.get(name=TEST_RESTAURANT_NAME)
        config = UpsellConfig.objects.create(restaurant=restaurant)
        item = self._make_item(image=_make_test_image())
        upsell = UpsellItem.objects.create(config=config, menu_item=item)
        data = UpsellItemSerializer(upsell).data
        self.assertIn('item_image_thumbnail', data)
        self.assertTrue(data['item_image_thumbnail'])
        self.assertIn('item_image', data)

    def test_backfill_command_populates_missing_thumbnails(self):
        from django.core.management import call_command
        item = self._make_item(image=_make_test_image())
        # Simulate the pre-feature state: thumbnail empty but image present.
        item.image_thumbnail.delete(save=False)
        MenuItem.objects.filter(pk=item.pk).update(image_thumbnail='')

        call_command('backfill_menu_thumbnails')
        item.refresh_from_db()
        self.assertTrue(item.image_thumbnail)

        # Second invocation is a no-op.
        thumb_name = item.image_thumbnail.name
        call_command('backfill_menu_thumbnails')
        item.refresh_from_db()
        self.assertEqual(item.image_thumbnail.name, thumb_name)
